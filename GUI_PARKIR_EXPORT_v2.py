...
def gen_qr():
        ...
        if(kondisi_kirim==True):
            ...
            transaction_redirect_url = str( transaction['redirect_url'])
        ...
        dta=transaction_redirect_url
        dta = create(dta)
        test = dta.xbm(scale=6)
...
while cap.isOpened():
    ...
    #op=cv2.resize(framew1,(320,247),cv2.INTER_AREA)
    op=cv2.resize(framew1, None, fx=0.5, fy=0.5, interpolation= cv2.INTER_AREA)  
    #op = cv2.resize(framew1,(224,224),fx=0,fy=0, interpolation = cv2.INTER_CUBIC)
    img3=cv2.cvtColor(framew,cv2.COLOR_BGR2RGB)
    img4=ImageTk.PhotoImage(Image.fromarray(img3))
    img31=cv2.cvtColor(op,cv2.COLOR_BGR2RGB)
    img41=ImageTk.PhotoImage(Image.fromarray(img31))
